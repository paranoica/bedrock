 ...
 *   (1) ROUTING DRIFT — a principle file exists but isn't listed in SKILL.md's file
 *       map (so the router never loads it), or the map names a file that's gone.
...
function check() {
  ...
  const { unrouted, dangling } = routingDrift();
  const out = {
    anti_slop_hash: cur,
    manifest_hash: man.anti_slop_hash,
    routing: { unrouted, dangling },
    stale_against_anti_slop: stale,
    clean: !unrouted.length && !dangling.length && !stale.length,
    note: "stale_against_anti_slop = principles last reconciled against an older anti-slop.md; " +
          ...
          "Advisory only — nothing is auto-edited.",
  };
