...
- **Q4 — 3D needed?** (fork): *no* → the whole 3D branch is skipped. *Yes* → level, material, asset source, **and production method**: *browser* (live React Three Fiber — the default; interactive / cursor / scroll-bound) or *baked* (offline headless Blender → PNG/EXR/webm/GLB; for path-traced cinematic stills, non-interactive loops, or heavy geometry baked to a light GLB). The decision table + the stager/harness rules live in `principles/3d-baked.md`; when in doubt, browser.
...
5. For 3D tasks, read `principles/3d.md`. If the survey routed 3D to the **baked** tier (Q4 production method), also read `principles/3d-baked.md` — the browser-vs-baked decision table, the stager-not-modeller rule, and the offline Blender harness (`tools/blender-render.md`, the measured sibling of `verify.md`).
